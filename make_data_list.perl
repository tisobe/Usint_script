...
#########################################################################################################
#													#
#	make_data_list.perl: make a data list for the current ao9 datasets				#
#													#
#	>>>>>>>>>> this version is specifically made for Ed Kellogg's need <<<<<<<<<<<<<		#
#													#
...
open(OUT, "> $temp_dir/zdata_list_temp");
...
#
#---- read obsid list for the current ao
#
...
system("rm /pool14/mta/ocat_data_list");
system("mv $temp_dir/zdata_list_temp  /pool14/mta/ocat_data_list");

################################################################################
### sub read_obsid_list: create obsid list for ao9			    ####
################################################################################
...
#--------------------------------------
        ...
        $sqlh1 = $dbh1->prepare(qq(select obsid from target
        where (obs_ao_str = "10") and (convert(int,seq_nbr) < 800000) 
                and (convert (int,seq_nbr) > 699999) 
                and (status = "unobserved" or status = "scheduled") 
                and grating = "NONE" and (instrument = "ACIS-S" or instrument = "ACIS-I")));
                $sqlh1->execute();
                $arrayref = $sqlh1->fetchall_arrayref();
                $sqlh1->finish;
